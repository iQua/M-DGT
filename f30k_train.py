...
''' Local Libs '''
from datasets import f30ke_base
from datasets import F30KE_provider
...
flags.DEFINE_string(
    "train_log_dir", os.path.join(CURRENT_PROJECT_DIR, "experiments",
                                  "FlikerE"),
    "the number of batches for one epoch")

flags.DEFINE_string(
    'visualization_dir',
    os.path.join(CURRENT_PROJECT_DIR, "visualization", "FlikerE"),
...
def _main(argv):
    f30k_bs = f30ke_base.F30KEBase(
        dataset_dir=FLAGS.Flickr30KEntities_source_path)
    ...
    f30ke_provider = F30KE_provider.F30KProvider(
        base_data=f30k_bs,
