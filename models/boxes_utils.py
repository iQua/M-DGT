...
def get_boxes_centers(batch_boxes):
    """[Get the centers of boxes]

    Args:
        boxes (list[torch.Tensor]): [a list of tensor in which each item is a tensor containing the boxes of the corresponding image,
                                    the type of box is <x_min, y_min, x_max, y_max>]
    """
    if not isinstance(batch_boxes, list):
        if isinstance(batch_boxes, np.ndarray):
            batch_boxes = torch.from_numpy(batch_boxes)
        batch_boxes = [batch_boxes]

    batch_boxes_centers = list()
    for boxes in batch_boxes:
        boxes_w = boxes[:, 2] - boxes[:, 0]
        boxes_h = boxes[:, 3] - boxes[:, 1]
        boxes_center_x = boxes[:, 0] + torch.div(boxes_w, 2)
        boxes_center_y = boxes[:, 1] + torch.div(boxes_h, 2)
        boxes_center_x = boxes_center_x.reshape((-1, 1))
        boxes_center_y = boxes_center_y.reshape((-1, 1))
        boxes_center = torch.cat([boxes_center_x, boxes_center_y], 1)
        batch_boxes_centers.append(boxes_center)

    return batch_boxes_centers
...
# this is the diou code obtained from the paper "Distance-IoU Loss: Faster and Better Learning for Bounding Box Regression".
...
def match(threshold, truths, priors, variances, labels, landms, loc_t, conf_t,
          landm_t, idx):
    ...
    matches = truths[
        best_truth_idx]  # Shape: [num_priors,4] 此处为每一个anchor对应的bbox取出来
    conf = labels[
        best_truth_idx]  # Shape: [num_priors]      此处为每一个anchor对应的label取出来
    conf[best_truth_overlap <
         threshold] = 0  # label as background   overlap<0.35的全部作为负样本
