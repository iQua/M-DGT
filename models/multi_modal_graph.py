...
from models.base_mm_graph_layer import CMGATLayerBase
...
class CMGATLayerImp(CMGATLayerBase):
    ...
    def __init__(self,
        ...
        super().__init__(cross_modal_attn_model, num_in_node_features,
                         num_in_phrase_features, num_out_node_features,
                         num_out_phrase_features, num_of_heads, concat,
                         activation, dropout_prob, add_skip_connection, bias,
                         log_attention_weights)

        self.cross_modal_node_encode_layer = cross_modal_node_encode_layer

    def compute_node_transform(self, multi_head_nodes_features,
    ...
    def forward(self, graph_data, text_data, mask):
        #
        # Step 1: node feature enhancement with cross-modal model
        #
        in_phrase_features = text_data
        in_nodes_features, in_nodes_boxes, edge_index, Hs, Ws = graph_data  # unpack data
        ...
        phrases_features_proj = self.dropout(phrases_features_proj)

        # nodes_phrases_response_scores is obtained by using the phrases as the query
        # Apply the scoring function (* represents element-wise (a.k.a. Hadamard) product)
        attn_text2visual_fs, nodes_phrases_response_scores = self.compute_node_transform(
            nodes_features_proj, phrases_features_proj, mask)

        # convert scores to the shape <batch_size, num_nodes, num_heads>
        nodes_phrases_response_scores = torch.squeeze(
            nodes_phrases_response_scores, 2)
        nodes_phrases_response_scores = torch.transpose(
            nodes_phrases_response_scores, 1, 2)

        # fusion the text to the visual to generate the enhanced node features
        encoded_cm_nodes_features_proj = self.cross_modal_node_encode_layer(
            visual_nodes_feas=nodes_features_proj,
            text_phrases_feas=attn_text2visual_fs)

        #
        # Step 1.1: add the node spatial information
        #
        nodes_spatial_coor = self.obtain_node_spatial_info(
            in_nodes_boxes, Hs, Ws)
        nodes_spatial_features = self.node_spatial_linear_proj(
            nodes_spatial_coor)

        # concat the two features to obtain the F_i feature of the node
        nodes_F_features = torch.cat(
            (encoded_cm_nodes_features_proj, nodes_spatial_features), 1)

        #
        # Step 2: Edge attention calculation
        #

        nodes_boxes_source, nodes_boxes_target = self.boxes_lift(
            nodes_boxes, edge_index)
        nodes_relative_coors = self.obtain_nodes_relative_spatial_info(
            nodes_boxes_source, nodes_boxes_target)

        # this edge features can be directly concated with the node features in our code
        #   the main reason is that they follow the same order based on edges index
        edges_features = self.edge_relative_spatial_linear_proj(
            nodes_relative_coors)

        # We simply copy (lift) the scores for source/target nodes based on the edge index. Instead of preparing all
        # the possible combinations of scores we just prepare those that will actually be used and those are defined
        # by the edge index.
        # scores shape = (E, NH), nodes_features_proj_lifted shape = (E, NH, FOUT), E - number of edges in the graph
        nodes_scores_source, nodes_features_proj_lifted = self.lift(
            nodes_phrases_response_scores, encoded_cm_nodes_features_proj,
            edge_index)
        ...
        #
        nodes_merged_features_lifted = torch.cat(
            (nodes_features_proj_lifted, edges_features), 1)
        # Element-wise (aka Hadamard) product. Operator * does the same thing as torch.mul
        # shape = (E, NH, FOUT) * (E, NH, 1) -> (E, NH, FOUT), 1 gets broadcast into FOUT
        nodes_merged_features_lifted_weighted = nodes_merged_features_lifted * attentions_per_edge

        # This part sums up weighted and projected neighborhood feature vectors for every target node
        # shape = (N, NH, FOUT)
        out_nodes_features = self.aggregate_neighbors(
            nodes_merged_features_lifted_weighted, edge_index,
        ...
        out_nodes_features = self.skip_concat_bias(attentions_per_edge,
                                                   in_nodes_features,
                                                   out_nodes_features)
        return (out_nodes_features, in_nodes_features, edge_index)
    ...
    def boxes_lift(elf, nodes_boxes, edge_index):
        """
        Lifts i.e. duplicates certain boxes depending on the edge index.
        """
        src_nodes_index = edge_index[self.src_nodes_dim]
        trg_nodes_index = edge_index[self.trg_nodes_dim]

        nodes_boxes_source = nodes_boxes.index_select(4, src_nodes_index)
        nodes_boxes_target = nodes_boxes.index_select(4, trg_nodes_index)

        return nodes_boxes_source, nodes_boxes_target
    ...
    def lift(self, nodes_response_scores, nodes_features_matrix_proj,
        ...
        nodes_scores_source = nodes_response_scores.index_select(
